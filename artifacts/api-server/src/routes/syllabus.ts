...
async function enrich(s: typeof syllabusBreakupsTable.$inferSelect) {
  const [teacher] = await db
    ...
    .from(usersTable)
    .where(eq(usersTable.id, s.teacherId))
    .limit(1);
  const [cls] = await db
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const filtered = academicYear
      ? all.filter((s) => s.academicYear === academicYear)
      : all;
    ...
    const byClassMap: Record<
      number,
      { classId: number; className: string; total: number; completed: number }
    > = {};
    for (const s of filtered) {
      if (!byClassMap[s.classId])
        byClassMap[s.classId] = {
          classId: s.classId,
          className: "",
          total: 0,
          completed: 0,
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    let rows = await db
      ...
      .from(syllabusBreakupsTable)
      .where(eq(syllabusBreakupsTable.tenantId, tenantId))
      .orderBy(syllabusBreakupsTable.month);
    ...
    if (academicYear)
      rows = rows.filter((r) => r.academicYear === academicYear);
  ...
  async (req, res): Promise<void> => {
    ...
    if (
      ...
      res
        ...
        .json({
          error:
            "branchId, classId, subjectId, academicYear, month, chapter, topic required",
        });
      return;
    }
    const tenantId = req.user!.tenantId;
    ...
    const [entry] = await db
      ...
      .values({
        ...
        expectedPeriods: expectedPeriods ?? null,
        plannedStartDate: plannedStartDate ?? null,
        plannedEndDate: plannedEndDate ?? null,
        learningOutcomes: learningOutcomes ?? null,
        teachingAids: teachingAids ?? null,
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const [entry] = await db
      ...
      .where(
        and(
          eq(syllabusBreakupsTable.id, id),
  ...
  async (req, res): Promise<void> => {
    ...
    if (!privileged) {
      if (existing.teacherId !== req.user!.userId) {
        res
          .status(403)
          .json({ error: "Cannot edit another teacher's syllabus entry" });
        return;
      }
      if (existing.status === "Completed") {
        res
          .status(403)
          .json({ error: "Cannot edit an approved syllabus entry" });
        return;
      }
    }
