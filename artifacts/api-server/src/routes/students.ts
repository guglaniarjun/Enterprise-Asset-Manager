...
async function enrichStudent(s: typeof studentsTable.$inferSelect) {
  ...
  const [sec] = await db
    ...
    .from(sectionsTable)
    .where(eq(sectionsTable.id, s.sectionId))
    .limit(1);
  const house = s.houseId
    ? (
        await db
          ...
          .from(housesTable)
          .where(eq(housesTable.id, s.houseId))
          .limit(1)
      )[0]
    : null;
  return {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (seen.has(admissionNo)) {
      errors.push({
        row: rowNum,
        field: "admission_no",
        message: `Duplicate admission_no in file: ${admissionNo}`,
        data: row,
      });
      continue;
    }
    ...
    const [dbStudent] = await db
      ...
      .where(
        and(
          ...
          eq(studentsTable.admissionNo, admissionNo),
        ),
      )
      .limit(1);
    if (dbStudent) {
      errors.push({
        row: rowNum,
        field: "admission_no",
        message: `Admission no already exists: ${admissionNo}`,
        data: row,
      });
      continue;
    }
    ...
    const [clsRow] = await db
      ...
      .from(classesTable)
      .where(
        and(eq(classesTable.tenantId, tenantId), eq(classesTable.name, cls)),
      )
      .limit(1);
    if (!clsRow) {
      errors.push({
    ...
    if (clsRow.branchId !== branchId) {
      errors.push({
        row: rowNum,
        field: "class",
        message: `Class ${cls} does not belong to selected branch`,
        data: row,
      });
      continue;
    }
    ...
    const [secRow] = section
      ? await db
          ...
          .where(
            and(
              eq(sectionsTable.classId, clsRow.id),
              eq(sectionsTable.name, section),
            ),
          )
          .limit(1)
      : await db
          .select()
          .from(sectionsTable)
          .where(eq(sectionsTable.classId, clsRow.id))
          .limit(1);
    ...
    if (!secRow) {
      errors.push({
        row: rowNum,
        field: "section",
        message: `Section not found: ${section} for class ${cls}`,
        data: row,
      });
      continue;
    }
    ...
    if (house) {
      const [houseRow] = await db
        ...
        .from(housesTable)
        .where(
          and(eq(housesTable.tenantId, tenantId), eq(housesTable.name, house)),
        )
        .limit(1);
    ...
    valid.push({
      ...
      fatherName:
        String(row.father_name ?? row.fatherName ?? "").trim() || null,
      motherName:
        String(row.mother_name ?? row.motherName ?? "").trim() || null,
      parentContact:
        String(row.parent_contact ?? row.parentContact ?? "").trim() || null,
      houseId,
      status: "active",
    });
  }
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    const tenantId = req.user!.tenantId;
    const { valid, errors } = await validateImportRows(
      rows,
      tenantId,
      Number(branchId),
    );
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    const [batch] = await db
      .insert(studentImportBatchesTable)
      .values({
        tenantId,
    ...
    const finalStatus =
      totalErrors === 0
        ...
        : successRows === 0
          ...
          : "partial";
    await db
      .update(studentImportBatchesTable)
      .set({ successRows, errorRows: totalErrors, status: finalStatus })
      .where(eq(studentImportBatchesTable.id, batch.id));
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const recentEvents = await db
      ...
      .orderBy(studentLogEventsTable.createdAt)
      .limit(20);
    const eventsEnriched = await Promise.all(
      recentEvents.map(async (e) => {
        ...
        return {
          ...
          admissionNo: enriched.admissionNo,
