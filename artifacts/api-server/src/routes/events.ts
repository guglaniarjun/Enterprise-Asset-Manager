...
async function enrich(e: typeof studentLogEventsTable.$inferSelect) {
  ...
  const [teacher] = await db
    ...
    .limit(1);
  const [log] = await db
    .select({ date: dailyClassLogsTable.date })
    .from(dailyClassLogsTable)
    .where(eq(dailyClassLogsTable.id, e.dailyLogId))
    .limit(1);
  return {
    ...e,
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const studentId = req.query.studentId
      ? parseInt(String(req.query.studentId), 10)
      : null;
    ...
    if (dateFrom || dateTo) {
      ...
      const logs = await db
        .select({ id: dailyClassLogsTable.id })
        .from(dailyClassLogsTable)
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (!dailyLogId || !studentId || !eventType) {
      res
        .status(400)
        .json({ error: "dailyLogId, studentId, eventType required" });
      return;
    }
    const tenantId = req.user!.tenantId;
    ...
    const [log] = await db
      ...
      .where(
        and(
          eq(dailyClassLogsTable.id, dailyLogId),
    ...
    if (!privileged && log.teacherId !== teacherId) {
      res
        .status(403)
        .json({ error: "Cannot add events to another teacher's log" });
      return;
    }
    ...
    if (
      ...
      res
        .status(409)
        .json({
          error: `Cannot add events to a log in state '${log.verificationStatus}'`,
        });
      return;
    }
    ...
    if (
      student.classId !== log.classId ||
      student.sectionId !== log.sectionId
    ) {
      res
        .status(400)
        .json({ error: "Student is not in the class/section of this log" });
      return;
    }

    // Transactional: event insert + audit row commit together so a partial
    // event write can never leak into the system. Audit failures inside the
    // transaction are still swallowed by writeAuditLog (best-effort logging)
    // but never roll back the primary insert.
    const event = await db.transaction(async (tx) => {
      const [inserted] = await tx
        .insert(studentLogEventsTable)
        .values({
          tenantId,
          dailyLogId,
          studentId,
...
router.patch(
  ...
  async (req, res): Promise<void> => {
    ...
    const [existing] = await db
      ...
      .where(
        and(
          eq(studentLogEventsTable.id, id),
