...
async function enrichAssignment(
  a: typeof teacherAssignmentsTable.$inferSelect,
) {
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const assignments = await db
      ...
      .where(eq(teacherAssignmentsTable.tenantId, tenantId));
...
router.post(
  ...
  async (req, res): Promise<void> => {
    const { branchId, teacherId, classId, sectionId, subjectId, academicYear } =
      req.body;
    if (
      !branchId ||
      !teacherId ||
...
router.patch(
  ...
  async (req, res): Promise<void> => {
    ...
    const [existing] = await db
      ...
      .where(
        and(
          eq(teacherAssignmentsTable.id, id),
