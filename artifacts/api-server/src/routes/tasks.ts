/**
 * @deprecated Legacy tasks router (Phase A). Superseded by /operations/tasks.
 *
 ...
 * task interactions go through /operations/tasks (see operations.ts).
 *
 * Every response includes:
 *   - `Deprecation: true`
 *   - `Sunset: <future date>`
 *   - `Link: </api/operations/tasks>; rel="successor-version"`
 *
...
// Tag responses ONLY for legacy CRUD handlers. router.use("/tasks", …) would
// prefix-match unrelated paths like /tasks/:id/source (a kpi.ts route) and
// stamp deprecation headers there too. Attach per-route instead.
const tagDeprecated = (req: Request, res: Response, next: NextFunction) => {
  res.setHeader("Deprecation", "true");
  res.setHeader("Sunset", "Thu, 31 Dec 2026 23:59:59 GMT");
  res.setHeader("Link", '</api/operations/tasks>; rel="successor-version"');
  next();
};
