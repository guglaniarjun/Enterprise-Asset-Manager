  ...
  const verifier = l.verifiedBy
    ? (
        await db
          .select({ name: usersTable.name })
          .from(usersTable)
          .where(eq(usersTable.id, l.verifiedBy))
          .limit(1)
      )[0]
    : null;
  return {
    ...
    const submitted = await db
      ...
      .where(
        and(
          ...
          eq(dailyClassLogsTable.date, dateStr),
        ),
      );
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const sectionId = req.query.sectionId
      ? parseInt(String(req.query.sectionId), 10)
      : null;
    ...
    const isTeacher =
      userRoleNames.includes(ROLES.TEACHER) &&
      !userRoleNames.some((r) =>
        [
          ...
          ROLES.PRINCIPAL,
          ROLES.COORDINATOR,
        ].includes(r as typeof ROLES.SUPER_ADMIN),
      );

    let rows = await db
      .select()
      .from(dailyClassLogsTable)
      .where(eq(dailyClassLogsTable.tenantId, tenantId))
      .orderBy(dailyClassLogsTable.date);
  ...
  async (req, res): Promise<void> => {
    ...
    if (
      ...
      res
        ...
        .json({
          error:
            "branchId, classId, sectionId, subjectId, date, periodNumber required",
        });
      return;
    }
    const tenantId = req.user!.tenantId;
    ...
    if (!isPrivileged(req) && !isCoordinator(req)) {
      ...
      if (!assignment) {
        res
          .status(403)
          .json({
            error: "You are not assigned to teach this class/section/subject",
          });
        return;
      }
    }
    ...
    const [log] = await db
      ...
      .values({
        ...
        topicPlanned: topicPlanned ?? null,
        topicTaught: topicTaught ?? null,
        syllabusStatus: syllabusStatus ?? "On Track",
        teachingMethod: teachingMethod ?? "Lecture",
        homeworkGiven: homeworkGiven ?? false,
        homeworkDetails: homeworkDetails ?? null,
        notebookWorkGiven: notebookWorkGiven ?? false,
        notebookWorkDetails: notebookWorkDetails ?? null,
        disciplineIssue: disciplineIssue ?? false,
        disciplineDetails: disciplineDetails ?? null,
        achievementDetails: achievementDetails ?? null,
        improvementDetails: improvementDetails ?? null,
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const rawEvents = await db
      ...
      .where(
        and(
          eq(studentLogEventsTable.dailyLogId, id),
          eq(studentLogEventsTable.tenantId, tenantId),
        ),
      );
    const studentEvents = await Promise.all(
      rawEvents.map(async (e) => {
        ...
        const [teacher] = await db
          .select({ name: usersTable.name })
          .from(usersTable)
          .where(eq(usersTable.id, e.teacherId))
          .limit(1);
        return {
          ...e,
  ...
  async (req, res): Promise<void> => {
    ...
    if (!privileged && existing.verificationStatus === "Pending") {
      res
        ...
        .json({
          error:
            "Log is awaiting verification; ask a coordinator to reject it first",
        });
      return;
    }
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (
      existing.verificationStatus !== "Draft" &&
      existing.verificationStatus !== "Rejected"
    ) {
      res
        .status(409)
        .json({
          error: `Cannot submit a log in state '${existing.verificationStatus}'`,
        });
      return;
    }
    ...
    const updatedRows = await db
      ...
      .where(
        and(
          eq(dailyClassLogsTable.id, id),
          eq(dailyClassLogsTable.tenantId, tenantId),
          inArray(dailyClassLogsTable.verificationStatus, [
            "Draft",
            "Rejected",
          ]),
        ),
    ...
    if (updatedRows.length === 0) {
      res
        .status(409)
        .json({
          error: "Log state changed concurrently; refresh and try again",
        });
      return;
    }
    ...
    await notifyRolesInBranch({
      ...
      roleNames: [ROLES.COORDINATOR, ROLES.PRINCIPAL],
  ...
  async (req, res): Promise<void> => {
    ...
    if (existing.verificationStatus !== "Pending") {
      res
        .status(409)
        .json({
          error: `Cannot verify a log in state '${existing.verificationStatus}' — only Pending logs can be verified`,
        });
      return;
    }
    ...
    await sendInAppNotification({
      ...
      title: "Log Verified",
      body: `Your class log for Period ${existing.periodNumber} has been verified.`,
      type: "success",
      relatedEntityType: "daily_log",
      relatedEntityId: id,
    });
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (!coordinatorRemarks) {
      res
        .status(400)
        .json({ error: "coordinatorRemarks required when rejecting" });
      return;
    }
    const [existing] = await db
      .select()
    ...
    if (existing.verificationStatus !== "Pending") {
      res
        .status(409)
        .json({
          error: `Cannot reject a log in state '${existing.verificationStatus}' — only Pending logs can be rejected`,
        });
      return;
    }
    ...
    await sendInAppNotification({
      ...
      userId: existing.teacherId,
      title: "Log Rejected",
      body: `Your log for Period ${existing.periodNumber} was rejected: ${coordinatorRemarks}`,
      type: "warning",
      relatedEntityType: "daily_log",
      relatedEntityId: id,
    });
