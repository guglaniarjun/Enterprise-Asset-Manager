...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const rows = await Promise.all(
      logs.map(async (l) => {
        ...
        return {
          ...
          "Topic Planned": l.topicPlanned ?? "",
          "Topic Taught": l.topicTaught ?? "",
          "Syllabus Status": l.syllabusStatus ?? "",
          "Teaching Method": l.teachingMethod ?? "",
          "Homework Given": l.homeworkGiven ? "Yes" : "No",
          "Notebook Work": l.notebookWorkGiven ? "Yes" : "No",
          "Discipline Issue": l.disciplineIssue ? "Yes" : "No",
          Remarks: l.remarks ?? "",
    ...
    await writeAuditLog({
      ...
      newValue: { dateFrom, dateTo, classId, count: logs.length },
      ipAddress: req.ip,
    });
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=daily-logs.xlsx",
    );
    res.send(buf);
  },
);
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const rows = await Promise.all(
      events.map(async (e) => {
        ...
        return {
          ...
          Severity: e.severity,
          Remarks: e.remarks ?? "",
          "Follow-up Required": e.followUpRequired ? "Yes" : "No",
    ...
    await writeAuditLog({
      ...
      tenantId,
      action: "EXPORT",
      entityType: "student_events",
      newValue: { eventType, count: events.length },
      ipAddress: req.ip,
    });
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=student-events.xlsx",
    );
    res.send(buf);
  },
);
