...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const missingLogsToday = Math.max(
      0,
      expectedLogsToday - submittedLogsToday,
    );
    ...
    const teacherCompliancePercent =
      teacherIds.length > 0
    ...
    const syllabusCompletionPercent =
      totalSyllabus > 0
        ...
        : 0;
    const classesBehindSyllabus = [
      ...new Set(
        syllabus.filter((s) => s.status === "Delayed").map((s) => s.classId),
      ),
    ].length;
    const subjectsBehindSyllabus = [
      ...new Set(
        syllabus.filter((s) => s.status === "Delayed").map((s) => s.subjectId),
      ),
    ].length;
    ...
    const disciplineCasesToday = logsToday.filter(
      (l) => l.disciplineIssue,
    ).length;
    ...
    for (const e of todayEvents.filter(
      ...
      studentDisciplineCount[e.studentId] =
        (studentDisciplineCount[e.studentId] ?? 0) + 1;
    }
    const repeatedDisciplineStudents = Object.values(
    ...
    for (const e of todayEvents.filter((e) => e.eventType === "Inattentive")) {
      studentInattentiveCount[e.studentId] =
        (studentInattentiveCount[e.studentId] ?? 0) + 1;
    }
    const repeatedInattentiveStudents = Object.values(
      studentInattentiveCount,
    ).filter((c) => c > 1).length;

    const achievementsRecorded = todayEvents.filter(
      (e) => e.eventType === "Achievement",
    ).length;
    const coordinatorVerificationPending = logsToday.filter(
      (l) => l.verificationStatus === "Pending" && l.submittedAt !== null,
    ).length;
    const rejectedLogsPendingResubmission = logsToday.filter(
      (l) => l.verificationStatus === "Rejected",
    ).length;

    const [{ total: openTasks }] = await db
      .select({ total: count() })
      .from(tasksTable)
      .where(
        and(eq(tasksTable.tenantId, tenantId), eq(tasksTable.status, "Open")),
      );
    const [{ total: criticalAlerts }] = await db
      .select({ total: count() })
      .from(alertsTable)
      .where(
        and(
    ...
    res.json({
      ...
      classesBehindSyllabus,
      subjectsBehindSyllabus,
      disciplineCasesToday,
      repeatedDisciplineStudents,
      repeatedInattentiveStudents,
      achievementsRecorded,
      coordinatorVerificationPending,
      rejectedLogsPendingResubmission,
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const rejectedLogs = logsToday.filter(
      (l) => l.verificationStatus === "Rejected",
    ).length;
    ...
    for (const l of logsToday)
      teacherLogCount[l.teacherId] = (teacherLogCount[l.teacherId] ?? 0) + 1;
    const repeatedNonComplianceTeachers = assignments.filter(
      (a) => !teacherLogCount[a.teacherId],
    ).length;

    const syllabusDelayedSubjects = (
      await db
        .select()
        .from(syllabusBreakupsTable)
        .where(
          and(
            ...
            eq(syllabusBreakupsTable.status, "Delayed"),
          ),
        )
    ).length;
    ...
    const events = await db
      ...
      .where(
        and(
          ...
          eq(studentLogEventsTable.severity, "High"),
        ),
      );
    const seriousDisciplineIssues = events.filter(
      (e) => e.eventType === "Discipline Issue",
    ).length;

    const coordinatorPendingVerifications = logsToday.filter(
      (l) => l.verificationStatus === "Pending" && l.submittedAt !== null,
    ).length;
    ...
    res.json({
      ...
      rejectedLogs,
      missingLogs,
      repeatedNonComplianceTeachers,
      syllabusDelayedSubjects,
      seriousDisciplineIssues,
      coordinatorPendingVerifications,
    });
  },
);
  ...
  async (req, res): Promise<void> => {
    ...
    const logsToday = await db
      ...
      .where(
        and(
          eq(dailyClassLogsTable.tenantId, tenantId),
          eq(dailyClassLogsTable.teacherId, teacherId),
          eq(dailyClassLogsTable.date, date),
        ),
      );
    ...
    const enrichedAssignments = await Promise.all(
      assignments.map(async (a) => {
        ...
        const [sec] = await db
          ...
          .from(sectionsTable)
          .where(eq(sectionsTable.id, a.sectionId))
          .limit(1);
        const [sub] = await db
    ...
    res.json({
      ...
      myRejectedLogs: allLogs.filter((l) => l.verificationStatus === "Rejected")
        .length,
      myUnverifiedLogs: allLogs.filter(
        (l) => l.verificationStatus === "Pending" && l.submittedAt !== null,
      ).length,
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const pendingLogs = allToday.filter(
      (l) => l.verificationStatus === "Pending" && l.submittedAt !== null,
    );
    const verifiedToday = allToday.filter(
      (l) => l.verificationStatus === "Verified",
    ).length;
    const rejectedToday = allToday.filter(
      (l) => l.verificationStatus === "Rejected",
    ).length;

    const enrichedPending = await Promise.all(
      pendingLogs.slice(0, 50).map(async (l) => {
        const [teacher] = await db
          .select({ name: usersTable.name })
          .from(usersTable)
          .where(eq(usersTable.id, l.teacherId))
          .limit(1);
        const [cls] = await db
          .select({ name: classesTable.name })
          .from(classesTable)
          .where(eq(classesTable.id, l.classId))
          .limit(1);
        const [sec] = await db
          .select({ name: sectionsTable.name })
          .from(sectionsTable)
          .where(eq(sectionsTable.id, l.sectionId))
          .limit(1);
        const [sub] = await db
          .select({ name: subjectsTable.name })
          .from(subjectsTable)
          .where(eq(subjectsTable.id, l.subjectId))
          .limit(1);
        return {
          ...l,
          teacherName: teacher?.name ?? "",
    ...
    res.json({
      date,
      pendingVerifications: pendingLogs.length,
      verifiedToday,
      rejectedToday,
      pendingLogs: enrichedPending,
    });
  },
);
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const dateFrom = String(
      req.query.dateFrom ?? new Date().toISOString().slice(0, 10),
    );
    const dateTo = String(
      req.query.dateTo ?? new Date().toISOString().slice(0, 10),
    );
    ...
    const logs = await db
      ...
      .where(
        and(
          eq(dailyClassLogsTable.tenantId, tenantId),
          gte(dailyClassLogsTable.date, dateFrom),
          lte(dailyClassLogsTable.date, dateTo),
        ),
      );
    const assignments = await db
      .select()
    ...
    const byTeacher = await Promise.all(
      Object.values(teacherMap).map(async (t) => {
        ...
        return {
          ...
          teacherName: u?.name ?? "",
          compliancePercent:
            t.expected > 0 ? Math.round((t.submitted / t.expected) * 100) : 100,
        };
      }),
    );
    ...
    const byClass = await Promise.all(
      Object.values(classMap).map(async (c) => {
        const [cls] = await db
          .select({ name: classesTable.name })
          .from(classesTable)
          .where(eq(classesTable.id, c.classId))
          .limit(1);
        return {
          ...c,
          className: cls?.name ?? "",
          compliancePercent:
            c.expected > 0 ? Math.round((c.submitted / c.expected) * 100) : 100,
        };
      }),
    );
  ...
  async (req, res): Promise<void> => {
    ...
    const events = await db
      ...
      .where(
        and(
          eq(studentLogEventsTable.tenantId, tenantId),
          eq(studentLogEventsTable.eventType, "Discipline Issue"),
        ),
      );
    ...
    for (const e of events) {
      const [log] = await db
        .select({ classId: dailyClassLogsTable.classId })
        .from(dailyClassLogsTable)
        .where(
          and(
            eq(dailyClassLogsTable.id, e.dailyLogId),
            eq(dailyClassLogsTable.tenantId, tenantId),
          ),
        )
        .limit(1);
      if (log) {
        if (!classMap[log.classId])
          classMap[log.classId] = { count: 0, logIds: [] };
        classMap[log.classId].count++;
      }
    }
    const byClass = await Promise.all(
      Object.entries(classMap).map(async ([classId, v]) => {
        const [cls] = await db
          .select({ name: classesTable.name })
          .from(classesTable)
          .where(eq(classesTable.id, parseInt(classId, 10)))
          .limit(1);
        return {
    ...
    for (const e of events)
      studentCount[e.studentId] = (studentCount[e.studentId] ?? 0) + 1;
    const repeatedStudents = Object.entries(studentCount)
      .filter(([, c]) => c > 1)
      .map(([id, count]) => ({ studentId: parseInt(id, 10), count }));
