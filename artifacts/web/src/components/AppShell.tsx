...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    {
      label: "Dashboard",
      href: `/dashboard/${user?.roles[0]?.roleName?.toLowerCase().replace(" ", "-") || "director"}`,
      roles: ["Director", "Super Admin", "Principal", "Coordinator", "Teacher"],
    },
    {
      label: "Logs",
      href: "/logs",
      roles: ["Director", "Principal", "Coordinator", "Super Admin"],
    },
    {
      label: "Missing Logs",
      href: "/logs/missing",
    ...
    },
    {
      ...
      roles: ["Director", "Principal", "Super Admin"],
    },
    { label: "Syllabus", href: "/syllabus", roles: ["Teacher", "Coordinator"] },
    {
      label: "Syllabus Summary",
      href: "/syllabus/summary",
      roles: ["Director", "Principal", "Super Admin"],
    },
    {
      label: "Students",
      href: "/students",
      roles: ["Director", "Principal", "Coordinator", "Teacher", "Super Admin"],
    },
    {
      label: "Events",
      href: "/events",
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-50 dark:bg-gray-900">
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-gray-800 border-r transition-transform transform ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} md:relative md:translate-x-0`}
      >
        ...
        <nav className="p-4 space-y-1 overflow-y-auto h-[calc(100vh-4rem)]">
          {allowedNavItems.map((item) => (
            ...
            >
              <span
                className={`block px-3 py-2 rounded-md text-sm font-medium cursor-pointer ${location.startsWith(item.href) ? "bg-primary/10 text-primary" : "text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800"}`}
              >
                {item.label}
              </span>
            </Link>
