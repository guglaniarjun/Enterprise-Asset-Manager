...
export default function SubmitLog() {
    ...
    if (existing && dashboard?.assignments) {
      const matched = dashboard.assignments.find(
        (a) =>
          ...
          a.sectionId === existing.sectionId &&
  ...
  const { data: syllabusListAll } = useListSyllabus(
    assignment
      ? { classId: assignment.classId, subjectId: assignment.subjectId }
      : undefined,
    {
      query: {
        enabled: !!assignment,
  ...
  useEffect(() => {
    ...
    if (s) {
      const planned = [s.chapter, s.topic, s.subtopic]
        .filter(Boolean)
        .join(" — ");
      if (planned && !form.getValues("topicPlanned"))
        form.setValue("topicPlanned", planned);
    }
  ...
  const addEvent = () => {
    ...
    const stu = studentsData?.data?.find(
      (s) => s.id.toString() === pickStudentId,
    );
    if (!stu) return;
    setEvents((prev) => [
      ...prev,
  ...
  async function onSubmit(values: LogValues) {
      ...
      updateLog.mutate(
        ...
        },
        {
          onSuccess: async () => {
            ...
            if (failed.length > 0) {
              toast({
                title: "Log updated — some events failed",
    ...
    if (!assignment) {
      toast({ title: "Pick an assignment", variant: "destructive" });
      return;
    }
    ...
    createLog.mutate(
      ...
      },
      {
        onSuccess: async (created) => {
          ...
          if (failed.length > 0) {
            toast({
              title: "Draft saved — some events failed",
              description: failed.join("; "),
              variant: "destructive",
            });
          } else {
            toast({
              title: "Draft saved",
              description: `Open the log and click "Submit for verification" when ready.${events.length ? ` ${events.length} student event(s) tagged.` : ""}`,
            });
          }
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input type="date" {...field} disabled={!!editId} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      Linked syllabus topic (optional — autofills topic planned)
                    </FormLabel>
                    <Select
                      onValueChange={field.onChange}
                      value={field.value ?? ""}
                      disabled={!assignment || syllabusOptions.length === 0}
                    >
                      <FormControl>
                        <SelectTrigger>
                          <SelectValue
                            placeholder={
                              !assignment
                                ...
                                : syllabusOptions.length === 0
                                  ? "No approved syllabus yet"
                                  : "Select planned topic"
                            }
                          />
                        </SelectTrigger>
                      </FormControl>
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        {...field}
                        placeholder="Lecture, Activity, Discussion, …"
                      />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormControl>
                          <Textarea
                            placeholder="Notebook work details…"
                            {...field}
                          />
                        </FormControl>
                      </FormItem>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormControl>
                          <Textarea
                            placeholder="Discipline issue details…"
                            {...field}
                          />
                        </FormControl>
                      </FormItem>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Tag absences, achievements, counselling needs, etc.
                    </p>
                  </div>
                  {events.length > 0 && (
                    <Badge variant="secondary">{events.length} tagged</Badge>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-2">
                  ...
                  >
                    <SelectTrigger className="md:col-span-2">
                      <SelectValue
                        placeholder={
                          assignment ? "Select student" : "Pick class first"
                        }
                      />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={addEvent}
                    disabled={!assignment || !pickStudentId}
                  >
                ...
                {events.length > 0 && (
                  <div className="space-y-1">
                    {events.map((ev, idx) => (
                      <div
                        key={idx}
                        className="flex items-center justify-between text-sm border rounded px-3 py-2 bg-background"
                      >
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Needs improvement</FormLabel>
                      <FormControl>
                        <Textarea rows={2} {...field} />
                      </FormControl>
                      <FormMessage />
              ...
              <div className="flex justify-end gap-2">
                <Button
                  ...
                  onClick={() =>
                    setLocation(
                      editId ? `/logs/${editId}` : "/dashboard/teacher",
                    )
                  }
                >
                  Cancel
