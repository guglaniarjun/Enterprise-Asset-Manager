...
export default function ClassSectionProfile() {
  ...
  if (error || !data)
    return <div className="text-destructive">Class/section not found.</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <h1 className="text-2xl font-bold">
        {data.class.name} — Section {data.section.name}
      </h1>
                ...
                <TableBody>
                  {data.syllabusProgress.map((s) => (
                    <TableRow key={s.subjectId}>
                      ...
                      <TableCell
                        className={s.delayed > 0 ? "text-destructive" : ""}
                      >
                        {s.delayed}
                      </TableCell>
                      <TableCell>
                        <Badge
                          variant={s.percent >= 80 ? "default" : "secondary"}
                        >
                          {s.percent}%
                        </Badge>
                      </TableCell>
