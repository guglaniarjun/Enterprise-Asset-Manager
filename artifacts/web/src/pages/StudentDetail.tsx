...
import { useGetStudent, useGetStudentEvents } from "@workspace/api-client-react";
...
export default function StudentDetail() {
  ...
  const { data: events, isLoading: eventsLoading } = useGetStudentEvents(id, undefined, {
    query: {
      enabled: !!id,
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <div className="text-sm font-medium text-muted-foreground">Admission No</div>
                <div className="text-base font-semibold">{student.admissionNo}</div>
              </div>
              <div>
                <div className="text-sm font-medium text-muted-foreground">Class & Section</div>
                <div className="text-base font-semibold">{student.className} {student.sectionName}</div>
              </div>
              <div>
                <div className="text-sm font-medium text-muted-foreground">Roll No</div>
                <div className="text-base font-semibold">{student.rollNo || '-'}</div>
              </div>
              <div>
                <div className="text-sm font-medium text-muted-foreground">House</div>
                <div className="text-base font-semibold">{student.houseName || '-'}</div>
              </div>
              <div>
                <div className="text-sm font-medium text-muted-foreground">Father's Name</div>
                <div className="text-base font-semibold">{student.fatherName || '-'}</div>
              </div>
              <div>
                <div className="text-sm font-medium text-muted-foreground">Mother's Name</div>
                <div className="text-base font-semibold">{student.motherName || '-'}</div>
              </div>
              <div className="col-span-2">
                <div className="text-sm font-medium text-muted-foreground">Parent Contact</div>
                <div className="text-base font-semibold">{student.parentContact || '-'}</div>
              </div>
              <div>
            ...
            <TableBody>
              {student.recentEvents?.map((event) => (
                <TableRow key={event.id}>
                  ...
                  <TableCell className="max-w-[200px] truncate" title={event.remarks || ''}>{event.remarks || '-'}</TableCell>
                  <TableCell>
