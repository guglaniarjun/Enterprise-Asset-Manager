...
export default function TaskDetail() {
  ...
  const taskQ = useQuery({
    queryKey: ["ops-task", id],
    queryFn: () => getOpsTask(id),
    enabled: !!id,
  });
  const actQ = useQuery({
    queryKey: ["ops-task-activity", id],
    queryFn: () => getOpsTaskActivity(id),
    enabled: !!id,
  });
  ...
  const statusMut = useMutation({
    mutationFn: ({ status, notes }: { status: string; notes?: string }) =>
      setOpsTaskStatus(id, status, notes),
    onSuccess: () => {
  ...
  const escalateMut = useMutation({
    mutationFn: ({ to, notes }: { to?: number; notes?: string }) =>
      escalateOpsTask(id, to, notes),
    onSuccess: () => {
  ...
  if (taskQ.error || !taskQ.data)
    return <div className="text-destructive">Failed to load task.</div>;
  const t = taskQ.data;
  ...
  return (
    <div className="space-y-6">
      <Breadcrumbs
        items={[{ label: "Tasks", href: "/tasks" }, { label: `#${t.id}` }]}
      />
      ...
      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
        <div>
          ...
          <div className="text-sm text-muted-foreground mt-1">
            ...
            Created {new Date(t.createdAt).toLocaleString()} by{" "}
            {t.createdByName ?? "—"}
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          {t.escalationLevel > 0 && (
            <Badge variant="destructive">Escalated L{t.escalationLevel}</Badge>
          )}
        </div>
      </div>
              ...
              <>
                ...
                <div>
                  ...
                  <div className="text-sm text-muted-foreground">
                    {formatHours(t.sla.hoursOver)} over · Due{" "}
                    {t.sla.dueAt ? new Date(t.sla.dueAt).toLocaleString() : "—"}
                  </div>
                </div>
              </>
            ) : (
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <Card className="md:col-span-2">
          ...
          <CardContent className="space-y-3 text-sm">
            <div>
              ...
              {t.description ?? <span className="italic">None</span>}
            </div>
            <div>
              <span className="text-muted-foreground">Assigned to:</span>{" "}
              {t.assignedToName ?? <span className="italic">Unassigned</span>}
            </div>
            <div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
              ...
              <>
                <div className="flex flex-wrap gap-2">
                  {allowed.includes("In Progress") && (
                    <Button
                      ...
                      variant="outline"
                      onClick={() =>
                        statusMut.mutate({ status: "In Progress" })
                      }
                  ...
                  {allowed.includes("Blocked") && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => statusMut.mutate({ status: "Blocked" })}
                      disabled={statusMut.isPending}
                    >
                      Block
                    </Button>
                ...
                <div>
                  ...
                  <Button
                    ...
                    disabled={statusMut.isPending || !allowed.includes("Done")}
                    onClick={() =>
                      statusMut.mutate({
                        status: "Done",
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      disabled={reassignMut.isPending || !reassignTo}
                      onClick={() => reassignMut.mutate(Number(reassignTo))}
                    >
                ...
                <div>
                  <div className="text-xs text-muted-foreground mb-1">
                    Escalate (optional target user ID)
                  </div>
                  <Input
                    value={escalateTo}
                  ...
                  <Button
                    ...
                    onClick={() =>
                      escalateMut.mutate({
                        to: escalateTo ? Number(escalateTo) : undefined,
            ...
            {terminal && (
              <div className="text-sm text-muted-foreground">
                This task is in a terminal state. No further actions.
              </div>
            )}
            <Separator />
            <Button
