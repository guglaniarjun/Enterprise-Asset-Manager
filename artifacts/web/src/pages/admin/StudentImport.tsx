...
const TEMPLATE_CSV = "admission_no,name,class,section,roll_no,father_name,mother_name,parent_contact,house\nADM001,Sample Student,Class 1,A,1,Father Name,Mother Name,9999999999,Red\n";
...
export default function StudentImport() {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      setRows(parsed);
      toast({ title: `${parsed.length} rows loaded`, description: "Click 'Validate' to preview." });
    } catch (err) {
      toast({ title: "Failed to read file", description: String((err as Error).message), variant: "destructive" });
    }
  };
  ...
  const runPreview = () => {
    ...
    if (rows.length === 0) { toast({ title: "Upload a CSV first", variant: "destructive" }); return; }
    preview.mutate({ data: { rows, branchId: parseInt(branchId, 10) } }, {
      onSuccess: (resp) => {
  ...
  const runConfirm = () => {
    ...
    confirm.mutate({ data: { rows: validRows, branchId: parseInt(branchId, 10), filename: filename || "import.csv" } }, {
      onSuccess: (resp) => {
        toast({ title: "Import complete", description: `${resp.successRows} students imported (${resp.errorRows} errors).` });
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Step 1 — Choose branch & upload file</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Columns: admission_no, name, class, section, roll_no, father_name, mother_name, parent_contact, house
              </p>
            </div>
          </div>
          ...
          <div className="flex justify-end">
            <Button onClick={runPreview} disabled={!branchId || rows.length === 0 || preview.isPending}>
          ...
          <CardContent className="space-y-4">
            <div className="flex flex-wrap gap-2">
              ...
              <Badge variant="destructive" className="text-sm">
                <AlertCircle className="w-3 h-3 mr-1" /> {previewErrors.length} errors
              </Badge>
              <Badge variant="outline" className="text-sm">{rows.length} total</Badge>
            </div>
                  ...
                  <TableBody>
                    {previewErrors.map((e, i) => (
                      <TableRow key={i}>
                        ...
                        <TableCell><code className="text-xs">{e.field ?? "-"}</code></TableCell>
            ...
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => { setPreviewDone(false); setRows([]); setValidRows([]); setPreviewErrors([]); if (fileRef.current) fileRef.current.value = ""; }}>
                Start over
              </Button>
              <Button
                onClick={runConfirm}
              ...
              >
                {confirm.isPending ? "Importing…" : `Confirm import (${validRows.length} students)`}
              </Button>
            </div>
          </CardContent>
        </Card>
